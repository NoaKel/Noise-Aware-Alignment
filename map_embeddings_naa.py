...
def main():
    ...
    mapping_type.add_argument('-hem', '--hard_em', action='store_true', help='use hard em mapping')
    mapping_type.add_argument('-sem', '--soft_em', action='store_true', help='use soft em mapping')
    ...
    while True:
        ...
        if args.orthogonal or it < 15: # orthogonal mapping
            print(it, 'orthogonal')
            ...
            print(it, 'unconstrained')
            x_pseudoinv = xp.linalg.inv(x[src_indices].T.dot(x[src_indices])).dot(x[src_indices].T)
            w = x_pseudoinv.dot(z[trg_indices])
            x.dot(w, out=xw)
            zw[:] = z
        elif (args.hard_em or args.soft_em) and not end: # hard or soft noise-aware alignment
            print(it, 'noise-aware alignment')
            is_soft = True if args.soft_em else False
            w, alpha, t_indices, f_indices = noise_aware(x[src_indices], z[trg_indices], is_soft)
            xw = x.dot(w)
            zw = z
            print ('number of clean pairs:', len(t_indices)) 
        else:  # advanced mapping
            print(it, 'advance mapping')
        ...
        if end:
            ...
            if args.direction == 'forward':
                ...
                idxs = list(zip(src_indices, trg_indices))
            ...
            if args.verbose:
                ...
                if args.validation is not None:
                    ...
                    f = open("accs_em.txt", "w", encoding="utf-8")
                    f.write('\n'.join(accs))
                    f.close
                    dict_sizes.append(str(len(src_indices)))
                    f = open("dict_sizes_em.txt", "w", encoding="utf-8")
                    f.write('\n'.join(dict_sizes))
                    f.close()
                sys.stderr.flush()
